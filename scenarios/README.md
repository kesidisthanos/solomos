...
Πρακτικοί κανόνες επιλογής:

1. **Όταν ο χρήστης ονομάζει μορφή ή πλατφόρμα, επιλέγεις σενάριο με βάση την τελική μορφή.**
   - επαγγελματικό email → `business-email.md`
   - bullet βιογραφικού → `cv-resume.md`
   - caption TikTok ή Instagram → αντίστοιχο σενάριο social ή Lexilog caption

2. **Όταν η οδηγία είναι αόριστη, συμπεραίνεις από το περιεχόμενο.**
   - «κάνε το καλύτερο» + κείμενο email → `business-email`
   - «διόρθωσε τα ελληνικά» + σελίδα προϊόντος → `product-description`
   - «μετάφρασέ το φυσικά» + αγγλικό πρωτότυπο → `translation-from-english`

3. **Όταν δύο σενάρια επικαλύπτονται, διαλέγεις βάσει τελικού παραδοτέου, όχι θέματος.**
   - LinkedIn post για προϊόν → σενάριο social/LinkedIn, όχι `product-description`
   - copy σελίδας προϊόντος → `product-description`, όχι `marketing-copy`
   - πειστικό email πώλησης → `sales-or-proposal`· απλή επαγγελματική επικοινωνία → `business-email`

4. **Ρωτάς διευκρίνιση μόνο όταν η πληροφορία που λείπει αλλάζει ουσιαστικά:**
   - τόνο
   - κοινό
   - δομή
   - ισχυρισμούς
...
5. **Αν παραμένει αβεβαιότητα, διαλέγεις το ασφαλέστερο πιθανό σενάριο και δηλώνεις σύντομα την υπόθεση** ώστε ο χρήστης να σε διορθώσει.
